...
export default function WorkoutsPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      if (sessionsRes.data) {
        setCompletedToday(sessionsRes.data.map((s) => s.workout_name))
      }
      setLoading(false)
    }
    fetchData()
    ...
    <div className="p-4 sm:p-6 lg:p-8 max-w-4xl mx-auto">
      <div className="mb-6">
        ...
        <p className="text-slate-500 mt-1">
          {workouts.length} אימונים מותאמים אישית עבורך
        </p>
      </div>
      ...
      {profile?.is_pregnant && (
        <div className="bg-pink-50 border border-pink-200 rounded-2xl p-4 mb-6">
          <div className="flex items-start gap-3">
            ...
            <div>
              <p className="text-pink-800 font-semibold text-sm mb-1">
                אימונים מאושרים להריון – שבוע {profile.pregnancy_week}
              </p>
            ...
            >
                ...
                <div className="flex items-start justify-between gap-3 mb-3">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 flex-wrap mb-2">
                      <span className={`text-xs font-medium px-2.5 py-1 rounded-full ${workoutTypeColors[workout.type]}`}>
                        {workoutTypeLabels[workout.type]}
                      </span>
                      <span className={`text-xs font-medium px-2.5 py-1 rounded-full ${intensityColors[workout.intensity]}`}>
              ...
              {workout.safetyWarnings && workout.safetyWarnings.length > 0 && (
                <div className="mx-5 mb-4 bg-amber-50 border border-amber-200 rounded-xl p-3">
                  <p className="text-amber-700 font-medium text-xs mb-1.5 flex items-center gap-1.5">
                    <AlertTriangle className="w-3.5 h-3.5" />
                    אזהרות בטיחות
                  </p>
                  <ul className="space-y-1">
                    {workout.safetyWarnings.map((w: string, i: number) => (
                      ...
                      <div key={i} className="flex items-start gap-3 bg-slate-50 rounded-xl p-3">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex flex-wrap gap-2 mt-1">
                            {ex.sets && (
                              <span className="text-xs text-slate-500">{ex.sets} סטים</span>
                            )}
                            {ex.reps && (
                              <span className="text-xs text-slate-500">× {ex.reps}</span>
                            )}
                            {ex.duration && (
                              <span className="text-xs text-slate-500">{ex.duration}</span>
                            )}
                            {ex.rest && (
                              <span className="text-xs text-slate-400">מנוחה: {ex.rest}</span>
                            )}
                            {ex.notes && (
                              <span className="text-xs text-slate-400 italic">{ex.notes}</span>
                            )}
                          </div>
                        </div>
                      </div>
              ...
              <div className="px-5 pb-5 flex items-center gap-3">
                ...
                >
                  {isExpanded ? (
                    <><ChevronUp className="w-4 h-4" /> הסתר תרגילים</>
                  ) : (
                    <><ChevronDown className="w-4 h-4" /> הצג תרגילים</>
                  )}
                </button>

                <button
                  onClick={() => !isDone && completeWorkout(workout)}
                  disabled={isDone || completing === workout.id}
                  className={`mr-auto flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-medium transition-all ${
                    isDone
